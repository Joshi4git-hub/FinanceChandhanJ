...
export const ProfilePage: React.FC = () => {
  const { user, profile, updateProfile, resetPasswordRequest, resetPasswordWithOtp } = useAuth();
  ...
  const handlePasswordResetRequest = async (e: React.FormEvent) => {
    ...
    const result = await resetPasswordRequest(user.email);
    setIsSendingResetCode(false);

    if (result.success) {
      setResetStatus({
        success: true,
        message: result.message || `Verification code sent to ${user.email}. Check your inbox!`,
      });
      setResetStep(2);
    } else {
      setResetStatus({
  ...
  const handlePasswordResetSubmit = async (e: React.FormEvent) => {
    ...
    const result = await resetPasswordWithOtp(user.email, resetOtp, newPassword);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-text-secondary dark:text-gray-400 mt-1">
              Manage your profile details, phone number, financial preferences, and security settings.
            </p>
          </div>
          {saveSuccess && (
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-[24px] shadow-soft border border-gray-100 dark:border-gray-700 text-center flex flex-col items-center">
              ...
              {phone && <p className="text-xs text-primary font-medium mt-1">{phone}</p>}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-[24px] shadow-soft border border-gray-100 dark:border-gray-700 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <Input
                  ...
                  value={user?.email || ''}
                  disabled
                  className="opacity-70 bg-gray-50 dark:bg-gray-900"
                />
                <Input
                  label="Phone Number"
                  type="tel"
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-[24px] shadow-soft border border-gray-100 dark:border-gray-700 space-y-6">
              <div className="flex items-center justify-between border-b border-gray-100 dark:border-gray-700 pb-4">
                ...
                <span className="text-xs px-3 py-1 bg-success/10 text-success font-semibold rounded-full flex items-center gap-1">
                  <Lock size={12} /> Protected
                </span>
              </div>

              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 bg-background dark:bg-gray-700/40 rounded-2xl border border-gray-100 dark:border-gray-700">
                <div>
                  <h4 className="text-sm font-semibold text-text-main dark:text-white flex items-center gap-2">
                    <KeyRound size={16} className="text-primary" /> Reset Account Password
                  </h4>
                  <p className="text-xs text-text-secondary dark:text-gray-400 mt-1">
                    Send a 6-digit OTP verification code to <strong>{user?.email}</strong> to set a new password.
                  </p>
                </div>
                <Button
                  type="button"
              ...
              <div className="space-y-4">
                <label className="flex items-center justify-between cursor-pointer p-3 rounded-2xl hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                  <div>
                    ...
                    <p className="text-xs text-text-secondary dark:text-gray-400">
                      Receive alerts for major budget thresholds and monthly summaries.
                    </p>
                  </div>
                  <input
                    type="checkbox"
                ...
                <label className="flex items-center justify-between cursor-pointer p-3 rounded-2xl hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                  <div>
                    ...
                    <p className="text-xs text-text-secondary dark:text-gray-400">
                      Instant browser alerts for upcoming EMI due dates.
                    </p>
                  </div>
                  <input
                    type="checkbox"
                ...
                <label className="flex items-center justify-between cursor-pointer p-3 rounded-2xl hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                  <div>
                    ...
                    <p className="text-xs text-text-secondary dark:text-gray-400">
                      Automated financial health report and personalized debt optimization tips.
                    </p>
                  </div>
                  <input
                    type="checkbox"
        ...
        {resetStep === 1 ? (
          <form onSubmit={handlePasswordResetRequest} className="space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <Button type="submit" isLoading={isSendingResetCode}>
          ...
          <form onSubmit={handlePasswordResetSubmit} className="space-y-4">
            <div className="p-3 bg-primary/10 border border-primary/20 rounded-xl text-xs text-primary font-medium flex items-center justify-between">
              <span>Code sent to <strong>{user?.email}</strong></span>
              <button
                type="button"
            ...
            {resetStatus.message && (
              <div
                className={`p-4 rounded-2xl text-sm font-medium flex items-center gap-2 ${
                  resetStatus.success ? 'bg-success/10 text-success' : 'bg-danger/10 border border-danger/20 text-danger'
                }`}
              >
                {resetStatus.success ? <CheckCircle size={18} /> : <AlertCircle size={18} className="shrink-0" />}
            ...
            <Input
              ...
              placeholder="123456"
              value={resetOtp}
              error={!resetStatus.success && resetStatus.message?.toLowerCase().includes('otp') ? resetStatus.message : undefined}
              onChange={(e) => {
                setResetOtp(e.target.value.replace(/\D/g, '').slice(0, 6));
                if (resetStatus.message) setResetStatus({});
              }}
              required
            />
            ...
            <Input
              ...
              value={newPassword}
              error={!resetStatus.success && resetStatus.message?.toLowerCase().includes('old password') ? resetStatus.message : undefined}
              onChange={(e) => {
