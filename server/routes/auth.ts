...
authRouter.post('/reset-password', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // 2. Check if new password is identical to current old password
    try {
      const { data: oldAuthCheck } = await supabase.auth.signInWithPassword({
        email: cleanEmail,
        password: newPassword,
      });
      if (oldAuthCheck?.session || oldAuthCheck?.user) {
        res.status(400).json({
          success: false,
      ...
      // Password check failed = new password is different from old password
    }
    ...
    try {
      ...
      while (!targetUserId && page <= 5) {
        ...
        const found = pageData.users.find((u) => u.email?.toLowerCase() === cleanEmail);
        if (found) {
      ...
      if (targetUserId) {
        ...
        if (!updateError) {
          ...
          console.warn('[Auth Server] Supabase updateUserById error:', updateError.message);
        }
      } else {
        // Create user in Supabase if not present
        const { data: newUser, error: createError } = await supabase.auth.admin.createUser({
          email: cleanEmail,
          password: newPassword,
          email_confirm: true,
        ...
        if (!createError && newUser?.user) {
          ...
          console.warn('[Auth Server] Supabase createUser error:', createError?.message);
        }
      }
    } catch (sbErr: any) {
      console.warn('[Auth Server] Supabase password update exception:', sbErr?.message);
    }
